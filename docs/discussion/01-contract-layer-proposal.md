...
> ステータス: **確定**（DD-006〜DD-013 として decisions.md に転記済み）
...
- 各論点の正式な決定は `decisions.md` の DD-006〜DD-013 を参照
- 本ファイルは推奨構成と理由・代替案の一覧として保持（今後の見直し時の出発点になる）
- 将来変更が必要になった場合は、新しい DD を追加し、本ファイルの該当行と decisions.md の旧 DD に「Superseded by DD-XXX」と注記する

## 将来再検討の余地がある項目（メモ）

- **Backend FW**: SSO/監査要件が強くなった場合 NestJS → 他検討、軽量化要望が出れば Hono へ移行も視野
- **認証**: 監査・SSO 要件で OAuth2/OIDC (Keycloak 等) に切替可能性あり
- **OpenAPI 配布**: 契約成果物の独立性を高めたくなった場合は専用リポジトリ化も検討
